...
if __name__ == '__main__':
    with app.app_context():
        db.create_all()
    # Mengubah debug=True menjadi debug=False untuk keamanan
    # app.run(host='0.0.0.0', port=5000, debug=True) <- Kode Lama
    app.run(host='0.0.0.0', port=5000, debug=False) # <- Kode Baru